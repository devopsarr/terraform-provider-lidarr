		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lidarr_download_client_nzbvortex.test", "host", "nzbvortex"),
					resource.TestCheckResourceAttr("lidarr_download_client_nzbvortex.test", "url_base", "/nzbvortex/"),
					resource.TestCheckResourceAttrSet("lidarr_download_client_nzbvortex.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lidarr_download_client_nzbvortex.test", "host", "nzbvortex-host"),
				),
			},
			// ImportState testing
			{
...
func testAccDownloadClientNzbvortexResourceConfig(name, host string) string {
	return fmt.Sprintf(`
	resource "lidarr_download_client_nzbvortex" "test" {
		enable = false
		priority = 1
		name = "%s"
		host = "%s"
