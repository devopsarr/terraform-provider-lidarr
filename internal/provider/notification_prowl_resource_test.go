		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lidarr_notification_prowl.test", "priority", "0"),
					resource.TestCheckResourceAttrSet("lidarr_notification_prowl.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lidarr_notification_prowl.test", "priority", "2"),
				),
			},
			// ImportState testing
			{
...
func testAccNotificationProwlResourceConfig(name string, priority int) string {
	return fmt.Sprintf(`
	resource "lidarr_notification_prowl" "test" {
		on_grab                            = false
		on_upgrade                         = false
