		...
		Steps: []resource.TestStep{
			...
			// Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.lidarr_delay_profile.test", "id"),
					resource.TestCheckResourceAttr("data.lidarr_delay_profile.test", "enable_usenet", "true")),
			},
		},
	})
}
...
func testAccDelayProfileDataSourceConfig(id int) string {
	return fmt.Sprintf(`
	data "lidarr_delay_profile" "test" {
		id = %d
	}
	`, id)
}
