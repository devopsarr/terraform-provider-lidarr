		...
		Steps: []resource.TestStep{
			...
			// Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.lidarr_quality_definition.test", "title"),
					resource.TestCheckResourceAttr("data.lidarr_quality_definition.test", "id", "21")),
			},
		},
	})
}
...
func testAccQualityDefinitionDataSourceConfig(id int) string {
	return fmt.Sprintf(`
	data "lidarr_quality_definition" "test" {
		id = %d
	}
	`, id)
}
