		...
		Steps: []resource.TestStep{
			...
			// Create and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lidarr_download_client_sabnzbd.test", "host", "sabnzbd"),
					resource.TestCheckResourceAttr("lidarr_download_client_sabnzbd.test", "url_base", "/sabnzbd/"),
					resource.TestCheckResourceAttrSet("lidarr_download_client_sabnzbd.test", "id"),
				),
			},
			// Unauthorized Read
			{
			...
			// Update and Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("lidarr_download_client_sabnzbd.test", "host", "sabnzbd-host"),
				),
			},
			// ImportState testing
			{
...
func testAccDownloadClientSabnzbdResourceConfig(name, host string) string {
	return fmt.Sprintf(`
	resource "lidarr_download_client_sabnzbd" "test" {
		enable = false
		priority = 1
		name = "%s"
		host = "%s"
